...
def r_squared(y, estimated):
    ...
    """
    yArr = np.array(y)
    estimatedArr = np.array(estimated)
    mean = sum(yArr) / len(yArr)
    estimatedError = ((yArr - estimatedArr) ** 2).sum()
    estimatedFluctuation = ((yArr - mean) ** 2).sum()
    return 1 - estimatedError / estimatedFluctuation
